...
pub fn setup_logging(config_file: &Path, base_path: &Path, default: &str) -> Result<String, Error> {
  println!("Initializing logging according to {:?}", config_file.to_str().unwrap_or("[??]"));

  if !config_file.exists() {
    if let Some(d) = config_file.parent() {
      fs
        ::create_dir_all(d)
        .map_err(|e| { Error::msg(format!("Could not create parent directory for log file: {:?}", e)) })?;
    };
  }
  ...
  let mut file = File::create(config_file).map_err(|e| {
    Error::msg(format!("Could not create default log file: {}", e.to_string()))
  })?;

  file.write_all(contents.as_bytes()).map_err(|e| Error::msg(format!("Could not write to file: {}", e.to_string())))?;
  Ok(contents)
}
